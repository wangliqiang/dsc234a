...
if ($_REQUEST['act'] == 'list') {
	...
	$msg_type = 0;
	$log_info = get_account_log_info($log_id);
	$frozen_money = $db->getOne('SELECT frozen_money FROM' . $ecs->table('seller_shopinfo') . 'WHERE ru_id = \'' . $log_info['ru_id'] . '\'');
	if ((0 < $log_status) && ($log_status <= 2)) {
		...
		else if ($log_info['frozen_money'] < $log_info['amount']) {
	...
	$links = array(
		array('href' => 'merchants_account.php?act=account_log_list', 'text' => $_LANG['account_list']),
