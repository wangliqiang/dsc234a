...
{
	$sql = 'SELECT gg.id, gg.goods_id, gg.group_id, CONCAT(g.goods_name, \' -- [\', gg.goods_price, \']\') AS goods_name ' . 'FROM ' . $GLOBALS['ecs']->table('group_goods') . ' AS gg, ' . $GLOBALS['ecs']->table('goods') . ' AS g ' . 'WHERE gg.parent_id = \'' . $goods_id . '\' ' . 'AND gg.goods_id = g.goods_id ';
